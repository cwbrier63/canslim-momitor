...
class ServiceController:
    ...
    def _init_discord(self):
        ...
        try:
            ...
            # Build webhooks dict from config
            webhooks = {}
            if discord_config.get('breakout_webhook_url'):
                webhooks['breakout'] = discord_config['breakout_webhook_url']
            if discord_config.get('position_webhook_url'):
                webhooks['position'] = discord_config['position_webhook_url']
            if discord_config.get('market_webhook_url'):
            ...
            self.discord_notifier = DiscordNotifier(
                webhooks=webhooks,
                default_webhook=discord_config.get('webhook_url'),
            )
    ...
    def _init_alert_service(self):
        ...
        try:
            ...
            self.logger.info(f"Alert service initialized (cooldown={'enabled' if alert_config.get('enable_cooldown', False) else 'disabled'})")
